...
## 10. 测量顺序:弥散的"双面" (2026-06-15, `pertoken_pool.py`)

pool-then-metric(池化向量再算几何)vs metric-then-pool(逐 token 算几何再池化),gsm8k L14:
| 指标 | pool-then-metric raw/桶 | metric-then-pool(mean) raw/桶 |
|---|---|---|
| **norm** | **0.775/0.707** | 0.573/0.602 |
| **pr** | 0.595/0.628 | **0.714/0.671** |
| **ae** | 0.582/0.629 | **0.717/0.677** |
| ed_half/e50/e90 | ~0.57/0.62 | ~0.71/0.67 |
- **顺序最优取决于指标 → 弥散有两张脸**:
  - **抵消面**(token 方向发散):norm/coherence/resultant **池化后测**最强;逐 token 模长平坦(0.57)= 个体不弱。
  - **逐token维度面**(每 token 各自激活更多维):pr/ae **逐token测再池化**最强(0.71),池化向量的有效维(0.58)把它抹掉。
- **但天花板没破**:融合 gsm8k 0.808(pool-then)→0.817(metric-then,+0.009);omnimath 0.819→0.820(+0.001)。逐token维度与 coherence/resultant/U_D **同轴冗余**,且同难度梯度(omnimath 桶内 ~0.53)。
- **第三次独立确认检测-AUROC 封顶 ~0.82**(R1 融合 / R2 富表征 / metric-then-pool 三路一致)。双面是好的机制叙事,不是数字突破。

## 11. 正交体检:resultant=coherence=norm 是同一信号 (2026-06-15, `signal_audit.py`)

**纠正第 7 节的隐含"distinct"**。步级相关矩阵(gsm8k L14):
| 对 | corr |
|---|---|
| resultant ↔ coherence | 1.00 |
| **resultant ↔ norm** | **0.94**(omnimath 0.97) |
| resultant ↔ cloud_D | −0.81 |
| resultant ↔ logn | −0.80 |
| **resultant ↔ U_D** | **−0.08**(omnimath +0.14,近正交) |
| cloud_D ↔ logn | **0.99** |
- **resultant 去掉 norm 后 AUROC 0.772→0.519(塌成随机)**;resultant over norm +0.003 ns;norm over resultant +0.004 ns。
- → **resultant / coherence / norm 是一个信号**(corr 0.94–0.97,互相零增量)。因 mean_tok_norm 近恒定,`norm ≈ const·resultant`。**resultant 在模型内判别力 = norm,无额外价值**;唯一剩余优势 = 跨模型无量纲可比(未测)。
- **真正的独立轴只有两个**:几何浓度(=norm=resultant) ⊥ 不确定性(U_D,corr≈0)。fuse 的 0.847 = 这两轴。**没有第三个几何轴。**
- cloud_D ↔ logn 0.99 再确认 = 长度。
- **几何特征空间被证明 ~1 维 + 长度**:第四次确认检测封顶,且知道了原因(无更多几何轴)。第7节"resultant 是干净估计器、norm 是污染代理"修正为:**判别等价的同一信号,resultant 仅是其无量纲写法**。
- 处置:norm×resultant 组合(共线 0.97,砍);expw−unif(resultant_unif 已加,预期≈0);massive(resultant_bulk 重抽收口)。

### massive 收口(resultant_bulk,去 top-5 massive 维,gsm8k)
| | resultant 全维 | resultant_bulk |
|---|---|---|
| 增量(over 混淆+熵) | +0.059 | **+0.055 [.036,.074] 显著** |
| raw L14 / 桶内 | 0.772 / 0.708 | 0.750 / 0.666 |
| corr 长度 | −0.83 | −0.906 |
- **去 massive 后信号基本不变 → 方向浓度不是 massive 平凡产物**(用户两次 massive 顾虑正式排除)。massive 中性(去掉略差、更长度相关),既非来源也非污染。
- **浓度家族最终定性:一个真实但中等的信号,与 norm 同体(去 norm→0.519),不靠 massive,难度梯度,封顶 ~0.7。**

### (a) 信号自身解耦:一个信号、bulk、非 massive (2026-06-15, `decouple_audit.py` + `--massive_global`)
全局固定 massive 维(预扫 60 正确链)+ norm_bulk/resultant_bulk 并排(修 per-chain 选维泄漏),gsm8k L14:
| | AUROC | corr 矩阵关键 |
|---|---|---|
| norm / resultant | 0.775 / 0.772 | corr(norm,resultant)=0.94 |
| **norm_bulk / resultant_bulk** | **0.744 / 0.752** | **corr(norm_bulk,resultant_bulk)=0.93** |
- 去 massive 后两者**都活**(只掉 ~0.02–0.03)、**corr 仍 0.93(不塌)**、互相增量 **ns**(+0.009/+0.000)。
- → **norm=resultant 是同一信号,住在 bulk 非 massive**(massive 仅贡献 ~0.02);幅度≡方向(mean_tok_norm 平)。**"能量vs方向两正交轴/多棱镜"不成立——一个几何轴。** 论文诚实写一个几何信号。

## 12. 几何地图完整:位移冗余 + 跨层无结构 (2026-06-15)

补测之前漏的两个几何家族(`mahal_step.py` / `layer_traj.py`),把"几何到头"从过早结论变成有证据结论。

### 位移(Mahalanobis/SPE 到健康流形)— 不是独立第二轴
| | raw | 桶内 | corr(·,norm) |
|---|---|---|---|
| SPE(leakage) | 0.691 | 0.684 | −0.38 |
| Mahalanobis | 0.591 | 0.540 | +0.31 |
- 位移在 **[长度+U+norm] 之上增量 +0.009 [−0.003,+0.018] ns** → **真但中等、被浓度+长度吸收,非独立轴**。(历史 SPE 0.65-0.69 复现,但同口径下不独立于浓度。)

### 跨层收敛形状 — 证伪(平凡解释胜出)
| | AUROC |
|---|---|
| 单层最优 | 0.771 |
| shape(slope/深−浅/曲率/range)单独 | 0.624 |
| 全 8 层 profile(线性袋) | 0.784 |
- **shape OVER 单层 = +0.000 [−0.005,+0.005] ns** → 收敛/形状**不是信号**;
- full-profile over base+单层 = **+0.023 SIG** = 纯**线性边际聚合**(平凡解释),非跨层结构。
- **收敛假设证伪**(累积态;Δ 版未测但先验极负:layer_coupling 协调 ns + 全层融合仅线性袋 + 此处 shape +0.000,三负)。

### 加权无关
resultant_unif 增量 +0.055 ≈ resultant +0.059 → exp 加权 = 等权,加权方案不影响。

### ★ 几何地图完整(论文硬通货)
**错误推理的几何信号 = 单一中等的浓度轴(norm=resultant=coherence)⊥ 不确定性(U_D);位移冗余、跨层无结构、cloud_D=长度。** 几何检测天花板 = 多层浓度(线性袋)+ 熵 ≈ 0.834。**没有第二几何轴。** 这是严格、完整、可发表的负面/审计结论。

## 13. 链内定位:真但弱,超长度超位置 (2026-06-15, `within_chain.py`)

用户 critique:池化 AUROC 可能全是链间难度效应,链内无法定位。测试(每错链:首错步 vs 该链**所有**其他步,含 post-error;position/n_tok 作基线;再对 n_tok 链内残差化)。
| metric | wc_loc | wc_loc(⊥nt) gsm8k | wc_loc(⊥nt) omni |
|---|---|---|---|
| coherence | 0.720 | **0.602** | 0.541 |
| resultant | 0.719 | 0.574 | 0.542 |
| norm | 0.706 | 0.548 | 0.536 |
| cloud_D | 0.722 | **0.510=随机** | 0.570 |
| U_D | 0.580 | 0.537 | **0.609** |
| position | **0.427** | — | — |
- **信号能链内定位(wc_loc 0.72 ≫ position 0.43,MRR 0.63 vs 随机 0.44)→ 不是纯链间难度效应**(用户担忧的最坏情况排除)。position 低于随机,证明非位置假象。
- **超出长度也成立但弱**:coherence/resultant ⊥nt = 0.57–0.60(gsm8k)/0.54(omni)>0.5 → 首错步比长度+位置匹配邻居更弥散,**经全混淆控制的真链内定位**。
- **cloud_D ⊥nt = 0.510 = 纯长度**(再确认死)。
- **几何/熵互补**:gsm8k 几何(coherence 0.60)>熵(0.54);omnimath 熵(U_D 0.61)>几何(0.54)。

### ★ 最终画像(论文核心)
错误推理的几何信号 = **单一方向浓度轴**(norm=resultant=coherence),**能在单条链里把首错步定位到比长度+位置匹配邻居更弥散处——真、但弱(wc_loc⊥nt ~0.55–0.60)、难度梯度、⊥不确定性、与熵互补**。其余(cloud_D/pr/ae/位移/跨层)=长度/冗余/无结构。**不是纯难度代理(链内测试推翻),也不是强定位器——是中等但全混淆控制的真链内定位信号。**

## 14. ★正面核心:几何⊥熵融合的链内首错定位器 (2026-06-15, `fuse_within.py`)

交叉拟合 logistic(按链 GroupKFold,detection 标签训练,预测全步含 post-error),评链内定位 wc_loc(⊥nt)=残差化掉 n_tok 后的链内定位。
| 特征集 | wc_loc(⊥nt) gsm8k | omnimath |
|---|---|---|
| geom 单独 | 0.587 | 0.619 |
| unc 单独 | 0.526 | 0.608 |
| **geom+unc** | **0.643** | **0.657** |
| all(+len,pos) | 0.625 | 0.589 |
| resultant only | 0.602 | 0.571 |
| U_D only | 0.529 | 0.608 |
- **geom+unc 同时超 geom 与 unc(各 ~+0.04),gsm8k+omnimath 双复现** → 单 config 内就有链级互补(有些链几何定位、有些熵定位),融合真增益。
- **难度鲁棒**:融合 0.643(易)/0.657(难),几乎不掉(甚至难数据略高)——**链内+融合消掉了池化检测的难度梯度**(易→几何,难→熵补)。
- **加长度反伤 ⊥nt**(all 0.625/0.589 < geom+unc)→ 长度只抬池化数,对超长度链内定位是负贡献。**定位器=geom+unc,不塞长度。**
- MRR 0.646/0.563 → 首错步平均排 ~1.5–1.8 位。**一个能用、难度鲁棒、全混淆控制的链内首错定位器。**

### 论文正面核心(从负面/审计升级)
**几何(方向浓度)⊥ 不确定性(熵)互补,融合成一个链内首错定位器:超长度、超位置、难度鲁棒(易难均 ~0.65)、复现。** 配审计骨架(eff-rank/cloud_D=长度、度量同源、位移/跨层无新轴),论文从"干净负面"升级为"负面审计 + 互补融合定位器"双结构。

## 15. 事件研究:同步为主,长链有前兆 (2026-06-15, `event_study.py`)

融合错误分数(残差化掉长度+位置,正确链交叉拟合)按距首错步偏移 Δ=j−k 的均值。correct 链基线 ≈0。
| Δ | gsm8k | omnimath |
|---|---|---|
| −5..−3 | ~+0.04 | +0.01→+0.02 |
| −2 | +0.023 | +0.033 |
...
| **0(错误步)** | **+0.108** | **+0.148** |
| +1 | +0.079 | +0.085 |
- **两 config 都有强同步峰**(Δ=0 明显最高,~2.5× 周围)→ 融合分数是干净的**同步定位器**。
- **omnimath(长/难链)错误前 3 步单调爬升**(+0.02→+0.03→+0.04)= **前兆**;**gsm8k(短链)平坦后跳** = 无前兆。→ **前兆数据依赖:长推理链才有"积累到出错"的空间**(且难题正是预警最有价值处)。
- 错误后衰减(滞后涟漪,正常)。
- 论文叙事:核心=同步链内定位;附加=长链上的早期预警动力学(前兆显著性待 bootstrap/within-chain 版确认)。
- **前兆显著性(event_study 终版)**:gsm8k 前兆 −0.004 **ns**(同步);**omnimath 前兆 +0.039 [0.030,0.049] 显著(长链有前兆)**。Q2 错步偏离 = gsm8k +0.49/中位0.67、omnimath +0.69/0.76 个链内 std(单步勉强→需 CUSUM)。两 config 错后衰减(局部失稳非持续坍缩)。

## 18. 共形 CUSUM 检测器:弱但有保证 (2026-06-16, `conformal_cusum.py`)

融合 geom⟂熵 分数 → 因果链内 z → CUSUM → 正确链 max-CUSUM 的 (1−eps) 分位做共形阈值 → 链级 FPR≤eps。
| config | eps | FPR | recall | delay中位 | early_warn |
|---|---|---|---|---|---|
| gsm8k | 0.05 | 0.058 | 14.6% | 0 | 17% |
| gsm8k | 0.20 | 0.221 | 39.0% | 0 | 24% |
| omnimath | 0.05 | 0.054 | 12.5% | 0 | **35%** |
| omnimath | 0.10 | 0.105 | 20.6% | 0 | **31%** |
- **FPR ≈ eps → 共形保证成立**(正确链误报受控,金融卖点);
- **recall 弱但有保证**:FPR≤5% ~13–15%,≤20% ~37–39%;delay 中位 0=同步;
- **early-warning:长链(omnimath)31–35% ≫ 短链(gsm8k)17–24%** → 前兆转化为错前报警;
- 指标用共形(FPR/recall/delay/早预警)非 AUROC——"有保证的弱检测器 > 无保证的强分数"。

### ★ 论文真实面貌(框法 B,数据支柱齐)
一个弱(recall~15%@FPR5%)但**有共形保证、零训练、跨域稳**的步级监控器 + 一张**失败动力学图**(错步同步弥散尖峰 / 长链前兆 / 错后衰减)。**机制(为什么/何时失败)> 检测数字(多准)**。

## 16. 预检:几何=链内, 熵=链间 (2026-06-15, `precheck_wcz.py`)

链内 z-score(过每链心化)后池化 AUROC,看 raw 里多少是链内 vs 链间(gsm8k L14):
| metric | raw | wcz(链内) | drop(链间) |
|---|---|---|---|
| norm | 0.775 | **0.696** | 0.079 |
| coherence | 0.778 | 0.692 | 0.086 |
| resultant | 0.772 | 0.678 | 0.094 |
| cloud_D | 0.714 | 0.629 | 0.085 |
| **U_D** | 0.619 | **0.352** | **0.268** |
- **几何(norm/resultant/coherence)链内保留 ~0.69**(只掉 ~0.08)→ 0.77 的大头是**链内**信号,不是链间难度。**地基稳**(否定"0.8 大半是难度"的担忧)。
- **U_D 链内塌到 0.352(跌破随机)**→ **输出熵几乎全是链间难度代理,不做链内定位**。强对比:**几何=链内错步定位器,熵=难度代理。**
- caveat:precheck 用 detection 标签(错步永远最后)→ 位置混淆(抬几何、压 U_D);干净版 within_chain ⊥nt 一致(geom 0.57–0.60,U_D 0.54/0.61),方向对。
- 论文加一句:几何误差信号**主要是链内的**(过链心化仍 ~0.69),输出熵**主要是链间的**——进一步区分二者,为"几何做链内定位"正名。
- omnimath 复现:norm wcz 0.705(drop −0.007,几乎零)、U_D wcz 0.519。**几何=链内、熵=链间,两 config 一致。**

## 17. Δ 跨层判决:结构证伪(最干净对象),Δ 非新轴 (2026-06-15, `delta_coupling.py`)

最小通道重抽(`--cloud_delta --layers all --no_ue --no_token_geom`,32 个相邻层增量),V*/μ 仅用完全正确链(无泄漏),三组 bag vs bag+c,raw+within-z,bootstrap 2000。gsm8k:
- **正交性:corr(resultant_delta, 累积 norm) = +0.796** → Δ 方向浓度**不是独立新轴**,是同一 ~0.8 浓度信号在增量上的化身。
- **协调度 c 全 ns**:norm_delta wcz +0.002 / resultant_delta +0.000 / **joint +0.006 [−0.000,+0.016] ns** → **非线性跨层协调对线性袋零增量**。
- **prefix 无前兆**:c(错误前缀) vs c(正确) 全 ±0.01(norm_delta −0.018 / res_delta +0.011 / joint −0.014)。
- 副产品:resultant_delta 线性袋 raw 0.802 / wcz 0.765 > 单层累积(0.678 wcz),但=多层线性聚合(同轴)非结构,且 wcz 有位置混淆 → 检测器微增益,非机制。

### ★ 跨层最终定论(四重否定)
**几何信号的跨层协调/结构不存在**:① 累积 c(layer_coupling)ns;② 全层融合只线性袋(+0.026);③ layer_traj shape over 单层 +0.000;④ **Δ 协调 c ns(剥恒等通路的最干净对象)**。**"谱场/跨层低秩协调"彻底证伪。** Δ 也不给新轴(corr 0.796)。跨层这条**完整封板**(只埋几何浓度的跨层组织;norm/模块协同的跨层是另命题,未做)。
