...
def cim_tle_intrinsic_dim(H: np.ndarray, k: int | None = None,
                          eps: float = 1e-12) -> float:
    """Nonlinear (kNN) intrinsic dimension of a token cloud, CIM-style.

    CIM uses a neighborhood-based intrinsic-dimension estimator (TLE family) in
    place of the linear effective rank. Here we use the Levina-Bickel maximum-
    likelihood estimator (MLE), a well-validated member of the same kNN-ID
    family. It is numerically far more stable than raw TLE on the *tiny* clouds
    we have (a single step often has only a handful of tokens), and recovers the
    true manifold dimension on controlled tests (2D-in-20D -> ~2, 5D-in-20D ->
    ~4-5), whereas a naive TLE implementation is extremely high-variance here.

    Per point z_i with k nearest-neighbor distances T_1 <= ... <= T_k:
        m_hat(z_i) = (k - 1) / sum_{j=1}^{k-1} log( T_k / T_j )
    The cloud-level estimate averages m_hat over points (with the standard
    inverse-average correction omitted for robustness on small n).

    Args:
        H: (n, p) token cloud (already projected to the analysis subspace).
        k: neighbors. Default min(n-1, 20). Capped at n-1.
        eps: numerical floor.

    Returns:
        scalar intrinsic dimension, or NaN if n < 4.

    NOTE: rationale for MLE over a hand-rolled TLE is purely numerical
    stability + correctness on small samples; both belong to the same kNN-ID
    family CIM draws on. If a reference TLE implementation is later required for
    exact parity with the paper, swap it in here -- the call sites do not change.
    """
    H = np.asarray(H, dtype=np.float64)
    if H.ndim != 2 or H.shape[0] < 4:
        return float("nan")
    n = H.shape[0]
    if k is None:
        k = min(n - 1, 20)
    ...
    for i in range(n):
        d = np.sort(Dmat[i])
        d = d[d > eps]          # drop self / coincident points
        if d.size < k:
            kk = d.size
        else:
            kk = k
...
def cim_information_volume(H: np.ndarray, eps: float = 1e-12) -> float:
    """CIM-style information volume of a token cloud (log-det form, Eqn. 14).

    V = 1/2 * log det( I + (p/n) Z Z^T )
      = 1/2 * sum_i log( 1 + (p/n) * lambda_i )

    where Z is the centered cloud (p x n), lambda_i are eigenvalues of Z Z^T
    (equivalently the squared singular values of the centered cloud), p is the
    feature dimension, n the number of tokens.

    This is the CORRECT "non-degenerate information" quantity from CIM. It is
    NOT the same as `spectral_energy` (= sum of squared singular values), which
    is dominated by the largest singular value and therefore measures
    *magnitude of the top direction* rather than *how many independent
    directions carry information*. The log(1 + .) compression saturates large
    directions and still counts small ones, so V grows with the number of
    effectively-occupied dimensions -- exactly the "does not collapse to a line"
    property CIM's anchor relies on.
    ...
    n, p = H.shape
    Hc = H - H.mean(axis=0, keepdims=True)
    # singular values of centered cloud; lambda_i = s_i^2
    s = np.linalg.svd(Hc, full_matrices=False, compute_uv=False)
    lam = s ** 2
    c = p / max(n, 1)
    return float(0.5 * np.sum(np.log1p(c * lam)))
...
def step_layer_cim_summary(H: np.ndarray,
                           tle_k: int | None = None,
                           ) -> tuple[float, float]:
    """One-shot CIM-style (D_tle, V_logdet) at a single (step, layer).

    D_tle:     TLE nonlinear intrinsic dimension (CIM Eqn. 6).
    V_logdet:  log-det information volume (CIM Eqn. 14).

    These are the CIM-faithful replacements for the linear effective rank and
    the spectral energy used in step_layer_spectral_summary. Computed directly
    from the token cloud H (already projected to the analysis subspace).
    """
    D_tle = cim_tle_intrinsic_dim(H, k=tle_k)
    V_ld = cim_information_volume(H)
    return D_tle, V_ld
