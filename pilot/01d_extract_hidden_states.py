"""
Extract per-step, per-layer HIDDEN STATE vectors from LLM reasoning traces.

Unlike 01c (which saves token-matrix SVD), this saves the actual last-token
hidden state h_{j,l} at each step j and layer l. This enables:
    - Trajectory-level intrinsic dimension (CIM-style)
    - Step-to-step displacement and direction analysis (TRACED-style)
    - Cross-layer difference analysis (TaT-style)

Output format per step:
    h_ml: (L, d) tensor — last-token hidden state at each layer
    sigma_ml: (L, k) tensor — token-matrix SVD (kept for backward compat)

Usage (on GPU server):
    python pilot/01d_extract_hidden_states.py \
        --model_path /gz-data/models/LLM-Research/Meta-Llama-3___1-8B-Instruct \
        --data_dir pilot/data \
        --splits gsm8k \
        --k 32 \
        --layers 0,3,7,11,15,19,23,27,31
"""
...
def process_example(example, model, tokenizer, num_layers, k=32, max_seq_len=2048,
                    layer_indices=None):
    """Extract per-step, per-layer hidden states + sigma.

    Returns list of dicts with:
        h_ml: (L, d) last-token hidden state per layer
        sigma_ml: (L, k) token-matrix SVD per layer
    """
    ...
    for j, (tok_start, tok_end) in enumerate(step_ranges):
        ...
        h_layers = []  # last-token hidden state per layer
        sigma_layers = []  # token-matrix SVD per layer

        for l in layer_indices:
            h_l = hidden_states[l + 1][0]  # +1 to skip embedding layer

            # Last-token hidden state for this step
            h_last = h_l[tok_end].float().cpu()  # (d,)
            h_layers.append(h_last)

            # Token-matrix SVD (backward compat)
            H_j_l = h_l[tok_start:tok_end + 1].float()  # (n_j, d)
            _, S, _ = torch.linalg.svd(H_j_l, full_matrices=False)
            sigma_l = torch.zeros(k)
            n_sv = min(len(S), k)
            sigma_l[:n_sv] = S[:n_sv].cpu()
            sigma_layers.append(sigma_l)

        h_ml = torch.stack(h_layers)       # (L, d)
        sigma_ml = torch.stack(sigma_layers)  # (L, k)
        ...
        results.append({
            "h_ml": h_ml,        # (L, d) — NEW: actual hidden states
            "sigma_ml": sigma_ml,  # (L, k) — kept for compat
            "is_error": is_error,
...
def main():
    parser = argparse.ArgumentParser(description="Extract hidden states + spectral features")
    parser.add_argument("--model_path", type=str, required=True)
    parser.add_argument("--data_dir", type=str, default="pilot/data")
    ...
    print(f"Per-step data: h_ml ({len(layer_indices)} x {hidden_dim}) + sigma_ml ({len(layer_indices)} x {args.k})")

    # Estimate storage: 9 layers x 4096 dims x 4 bytes x ~5 steps x ~400 examples ≈ 300 MB
    est_mb = len(layer_indices) * hidden_dim * 4 * 5 * 400 / (1024**2)
    ...
    for split in args.splits.split(","):
        ...
        out_path = os.path.join(args.output_dir, f"{split}_hidden_states.pt")
        save_data = {
            "examples": all_data,
            "meta": {
                "k": args.k,
                ...
                "extraction": "last_token_hidden_state",
            }
        }
