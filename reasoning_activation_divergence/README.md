...
原始残差流与 task-probe Fisher 管线都以 `scikit-learn` 为正式依赖：共享投影
使用 `sklearn.utils.extmath.randomized_svd`，仿射输运和探针分别使用
`sklearn.linear_model.Ridge` 与 `LogisticRegression`，不提供退化替代实现。
