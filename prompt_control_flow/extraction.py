...
from .metrics import compute_step_residual_vectors, compute_step_state_vectors, summarize_step_metrics
...
def extract_chain_mechanisms(
    ...
    if not extractors:
        raise ValueError("at least one mechanism extractor is required")
    prompt, response = build_prompt_response(record.problem, record.steps)
    ...
    for extractor in extractors:
        scores = extractor.compute(cache, record, cfg)
    ...
    if cfg.store_step_state_vectors and cache.hidden_states is not None:
        hidden = [np.asarray(h, dtype=np.float32) for h in cache.hidden_states]
        step_state_vectors = compute_step_state_vectors(
            hidden,
            step_ranges=cache.step_token_ranges,
            layers=cfg.layers,
        )
...
def pack_extractions(extractions: Sequence[MechanismExtraction]) -> Dict[str, np.ndarray]:
    ...
    packed = {
        ...
        "metadata_json": np.asarray([str(e.metadata) for e in extractions], dtype=object),
    }
    if flat_vectors:
