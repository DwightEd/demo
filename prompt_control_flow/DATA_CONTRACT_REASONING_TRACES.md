...
Hidden-state depths use one convention throughout this project. If
`hidden_states[0]` is the embedding output, depth \(d\geq 1\) is the state
after transformer block \(d\), and the block update entering that depth is

\[
\Delta h_i^{(d)} = h_i^{(d)} - h_i^{(d-1)}.
\]

The extractor stores three explicitly named step views. For inclusive step
range \([a,b]\), they are

\[
h_{a-1}^{(d)}\quad\text{(causal pre-step)},\qquad
\frac{1}{b-a+1}\sum_{i=a}^{b}h_i^{(d)}\quad\text{(retrospective mean)},
\qquad h_b^{(d)}\quad\text{(retrospective end)}.
\]

Only the first may be used to predict whether the upcoming step will fail.
...
```text
gold_error_step       ProcessBench index; -1=all steps valid; -2=unavailable
...
dataset, subset, split, problem_id, problem_group_id, sample_idx
generator                         model that generated the candidate answer text
source_model, source_model_revision       observer that encoded this exact axis
source_tokenizer, source_tokenizer_revision
...
Every new mechanism artifact is round-trip replayable: the writer's output
must pass the same exact-trace loader without retokenization. It additionally
records `problem_group_id`, `process_correct`, `final_answer_correct`,
`replay_protocol`, and the prompt/question/response token ranges.

`generator` and `source_model` must never be conflated: a published candidate
may have been generated by one model and teacher-forced through a different
observer. Exact token-ID compatibility checks use `source_model` and
`source_tokenizer`, while generator-stratified analysis uses `generator`.

For public ProcessBench rows, the primary observer protocol is
`processbench_observer_chat_v1`: a frozen solver instruction rendered through
the loaded instruction model's chat template. `processbench_observer_plain_v1`
is retained only as a legacy prompt ablation. Neither is presented as the
unknown original generator prompt. Exact self-generation artifacts always use
`exact_artifact_replay` and bypass prompt rendering and retokenization.
...
The current generic ICR extractor is short-sequence only and fails closed above
`full_attention_token_threshold`; it does not claim an exact layerwise long-
sequence path.
...
Exact benchmark-observer pilot over ProcessBench. The chat-template observer is
the primary protocol; the plain `Problem/Solution` prompt is a declared legacy
ablation, never the unknown original generation prompt:
...
python extract_mechanisms.py \
  ...
  --subset gsm8k \
  --model /gz-data/models/Meta-Llama-3.1-8B-Instruct \
  --output data/exact/processbench_gsm8k_observer.npz \
