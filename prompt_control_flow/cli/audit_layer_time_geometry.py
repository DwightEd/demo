...
def render_input_report(status: dict[str, object]) -> str:
    ...
    lines = [
        ...
        f"layer metadata key: {status.get('layer_time_layer_key', '') or 'none'}",
        f"layers: {layers}",
        f"stored stepvec depth: {status.get('layer_time_stored_stepvec_depth', 0)}",
        f"layer metadata matches state: {status.get('layer_time_layer_metadata_matches_state', True)}",
