"""Cross-fit Bayes-LR (rules out in-sample KDE optimism) + error-rate by kappa decile within mid-length (U-shape?)."""
from __future__ import annotations
import argparse
import numpy as np


def auroc(s, y):
    m = np.isfinite(s); s, y = s[m], y[m]
    p, n = int((y == 1).sum()), int((y == 0).sum())
    if not p or not n:
        return float("nan")
    o = np.argsort(s, kind="mergesort"); r = np.empty(len(s)); sr = s[o]; i = 0
    while i < len(sr):
        j = i
        while j + 1 < len(sr) and sr[j + 1] == sr[i]:
            j += 1
        r[o[i:j + 1]] = 0.5 * (i + j) + 1; i = j + 1
    return (r[y == 1].sum() - p * (p + 1) / 2) / (p * n)


def bdir(a):
    return max(a, 1 - a) if np.isfinite(a) else a


def kde(x, grid, bw):
    x = x[np.isfinite(x)]
    if len(x) < 5:
        return np.full(len(grid), np.nan)
    d = (grid[:, None] - x[None, :]) / bw
    return np.exp(-0.5 * d * d).sum(1) / (len(x) * bw * np.sqrt(2 * np.pi))


def lr_xfit(k, y, seed=0):
    m = np.isfinite(k); k, y = k[m], y[m]
    if y.sum() < 10 or (y == 0).sum() < 10:
        return float("nan")
    rng = np.random.default_rng(seed); idx = rng.permutation(len(k)); h = len(k) // 2
    s = np.zeros(len(k))
    for a, b in ((idx[:h], idx[h:]), (idx[h:], idx[:h])):
        bw = 1.06 * k[a].std() * len(a) ** (-1 / 5) + 1e-9
        p0 = kde(k[a][y[a] == 0], k[b], bw); p1 = kde(k[a][y[a] == 1], k[b], bw)
        s[b] = (p1 + 1e-12) / (p0 + 1e-12)
    return auroc(s, y)


def main():
    ap = argparse.ArgumentParser(); ap.add_argument("npz"); ap.add_argument("--layer", type=int, default=14)
    ap.add_argument("--nbk", type=int, default=6); args = ap.parse_args()
    z = np.load(args.npz, allow_pickle=True)
    cn = [str(x) for x in z["cloud_feature_names"]]; lyu = [int(x) for x in z["layers_used"]]; li = lyu.index(args.layer)
    fi = cn.index("resultant"); SC, SR = z["stepcloud"], z["step_token_ranges"]; ges = z["gold_error_step"].astype(int)
    K, Y, NT = [], [], []
    for i in range(len(SC)):
        sc = np.asarray(SC[i], float); rng = np.asarray(SR[i], int); k = int(ges[i]); correct = k < 0
        for j in range(rng.shape[0]):
            if correct or j < k:
                lab = 0
            elif j == k:
                lab = 1
            else:
                continue
            K.append(float(sc[j, li, fi])); Y.append(lab); NT.append(int(rng[j, 1] - rng[j, 0] + 1))
    K = np.asarray(K); Y = np.asarray(Y, int); NT = np.asarray(NT, float)

    print(f"{args.npz} | L{args.layer} | steps {len(Y)} err {int(Y.sum())}")
    print(f"  raw:  monotone {bdir(auroc(-K, Y)):.3f} | Bayes-LR xfit {bdir(lr_xfit(K, Y)):.3f}")
    e = np.quantile(NT, np.linspace(0, 1, args.nbk + 1)); e[-1] += 1
    b = np.clip(np.digitize(NT, e[1:-1]), 0, args.nbk - 1); num = den = mn = 0.0
    for bb in range(args.nbk):
        mm = b == bb; ne, ng = int(Y[mm].sum()), int((Y[mm] == 0).sum())
        if ne < 10 or ng < 10:
            continue
        al = lr_xfit(K[mm], Y[mm])
        if np.isfinite(al):
            num += al * ne * ng; den += ne * ng; mn += bdir(auroc(-K[mm], Y[mm])) * ne * ng
    if den:
        print(f"  len-ctrl: monotone {mn/den:.3f} | Bayes-LR xfit {num/den:.3f}")

    mid = (NT >= np.quantile(NT, 1 / 3)) & (NT < np.quantile(NT, 2 / 3))
    km, ym = K[mid], Y[mid]; q = np.quantile(km, np.linspace(0, 1, 11)); q[-1] += 1e-9
    print("  mid-length error-rate by kappa decile (low->high kappa):")
    rates = [float(ym[(km >= q[d]) & (km < q[d + 1])].mean()) for d in range(10)]
    print("   " + " ".join(f"{r:.2f}" for r in rates))


if __name__ == "__main__":
    main()
