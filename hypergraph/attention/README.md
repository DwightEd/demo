...
`inspect/build` 负责 token/step 对齐、causal attention、构图和 schema，不代替 shard 审计；若未用
双卡脚本，先手动执行 `python -m hypergraph.attention.shards <trace-dir> ...`。只有诊断任务才使用
...
每条 trace 同时保存 model/template/dtype/layer-head、replay mode/fidelity、prompt 来源和
prompt/response hash。训练会检查所有监督样本的方法 fingerprint 与原模型 axis id 完全
一致；输入文件 SHA256、原始行号和可复算的 scope fingerprint 则单独保存，因此互补 shard
