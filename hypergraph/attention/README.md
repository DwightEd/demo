...
每个 response receiver \(i\)、layer \(l\) 和 head \(h\)，默认构造：

\[
e_{i,l,h}=\{i\}\cup\{j<i:A_{l,h,i,j}>0.01\}.
\]

至少存在一个历史 source 才保留该超边。默认方法的其余约束是：

- 所有 prompt 与 response 历史 token 均可成为 source；
- 不使用 top-k，只有 attention threshold；
...
这对应以下 resolved 默认配置（`target_alignment` 是冻结的 schema 字段，不是 CLI 开关）：

```text
threshold=0.01                 source_selection=threshold, top_k=None
min_sources=1                 include_center=true
source_scope=all_past         incidence_weight_mode=uniform
propagation_mode=symmetric    edge_attr_mode=faithful (3-D)
...
mlp_norm=true                 receiver_source_interaction=false
dropout=0.25                  learning_rate=3e-4
weight_decay=1e-3             scheduler=cosine, warmup=0.05
grad_accumulation=1           pooling=mean
```

注意：`min_sources` 只计算历史 source，不包含 receiver。原脚本的
`MIN_MEMBERS_IN_HE=2` 且 `INCLUDE_CENTER_TOKEN=True`，恰好等价于默认
`min_sources=1`。
...
threshold 也不是原仓库唯一常量：当前 `processed_hypergraph.py` 的活动入口使用 `0.01`，
另一个 attribute 脚本和 multi-batch QA 路径写的是 `0.05`。本目录默认复现活动单图组合
(`tau=0.01 + per_graph_zscore + classifier LayerNorm + Xavier init`)。multi-batch 入口除
...
基线不需要保存 hidden。trace 默认以 float32 保存 attention，避免在 `tau=0.01`
附近因额外 fp16 量化翻转成员关系；只有经过边稳定性审计后才使用
...
`outputs/attention_hypergraph/<dataset>_response_layer<layer>_matched_Llama-3.1-8B-Instruct`；
all-generator observer 使用 `_observer_all` 后缀。若必须新抽 matched cohort，trace 目录也使用
对应 `_matched_...` 后缀；最终五折/多种子汇总为
`aggregate_results.json`。其中 `test_aggregate` 是总体 held-out 指标，
`generator_test_aggregate` 保存每个 generator 在各 fold/seed 上的样本数、类别数和分层指标；
单 generator 子集中个别折若只有单类，对应分层 AUROC/AUPR 会明确为 `null`，不会伪造数值。
当前分层汇总是“指标有定义的 fold/seed 的均值”，并记录 `n_defined_runs`，不是把多 seed 的
重复 held-out prediction 拼接成 pooled OOF 指标；论文分析应另行生成按 seed 去重的 pooled OOF。
...
通道。`STORAGE_DTYPE=float16` 也必须先检查 `tau=0.01` 周围的成员稳定性；这些设置均不能
...
缓存分块门禁只绑定 `threshold` 且无训练期 `top-k` 的构图配置，并且训练阈值必须与抽取门禁
阈值完全一致。`top_k_only`、`cumulative_mass`、threshold 后再 top-k 或其他 threshold sweep
...
|---|---|---|---|
| attention 拓扑选择 | 全 layer/head | `--selected-layers`、`--selected-heads` | 这里只删除对应超边，node diagonal 仍保留全部通道，从而隔离拓扑贡献；需要 fold 内选，避免测试集挑 head |
| 超边稀疏化 | `--source-selection threshold --threshold 0.01` | threshold 后 `--top-k`；`top_k_only`；`cumulative_mass` | 原仓库存在两种 tau；后两者控制长度稀疏性但改变原拓扑 |
...
目前 CLI 已实现单 fold、单 seed 的训练与预测接口，CPU/PyTorch 路径和模拟 cache 分块路径
已通过作用域测试；真实 Llama checkpoint 的显存、吞吐和分块等价性仍必须由上述服务器 pilot
实测。多 fold/seed 可以由实验队列分别
调用，并在 `predictions_test.csv` 上统一做 problem-level bootstrap。逐 step 预测同时
导出预先对齐的 token 长度；长度 bins 必须在查看 test 结果前固定。尚未运行真实数据实验，
所以本目录不声称任何效果提升。
